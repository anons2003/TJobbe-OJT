...
//    }
 @PostMapping("/save")
 public ResponseEntity<?> saveJob(@RequestBody Job jobRequest) {
     try {
         // Lưu công việc vào cơ sở dữ liệu
         Job savedJob = jobRepository.save(jobRequest);

         // Lấy ID doanh nghiệp từ công việc đã lưu
         int enterpriseId = savedJob.getEnterprise().getEid();

         // Gửi email thông báo lưu công việc
         emailService.sendEmailToJobSave(savedJob, enterpriseId);
...
//    }
    ...
    public ResponseEntity<?> deleteJob(@PathVariable("jobId") Long jobId) {
        try {
            ...
            if (jobOptional.isPresent()) {
                Job job = jobOptional.get();
                int enterpriseId = job.getEnterprise().getEid();
                jobPostService.deleteJob(jobId);
                emailService.sendEmailToJobDelete(job, enterpriseId);
    ...
    @GetMapping("/monthly-job-posts")
    public ResponseEntity<List<MonthlyJobPostDTO>> getMonthlyJobPosts(@RequestParam int year) {
        List<MonthlyJobPostDTO> monthlyJobPosts = jobPostService.getMonthlyJobPosts(year);
        return ResponseEntity.ok(monthlyJobPosts);
    }
