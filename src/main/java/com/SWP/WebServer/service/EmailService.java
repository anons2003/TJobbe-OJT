...
public class EmailService {
    ...
    public void sendMail(
        ...
        helper.setFrom("tjob.noreply@gmail.com");
        helper.setTo(email);
    ...
    // delete job
    public void sendEmailToJobDelete(Job job, int eid) {
        Enterprise enterprise = enterpriseRepository.findByEid(eid);
        ...
        if (enterprise != null) {
            ...
            String jobTitle = job.getTitle();
            String name = "Top Job";
            String subject = "Job Deleted Notification";
            String body = String.format(
                    "Dear %s,%n%n" +
                            "We would like to inform you that your job posting titled '%s' has been deleted from the platform.%n%n" +
                            "If you have any questions, feel free to contact us.%n%n" +
                            "Best regards,%n" +
                            "%s",
                    enName, jobTitle, name
            );
            sendEmailToEnterprise(eid, name, toEmail, subject, body);
        } else {
            // Handle case where enterprise is not found
    ...
    public String sendEmailToAdmin(int eid, String name, String email, String subject, String body) {
        Enterprise enterprise = enterpriseRepository.findByEid(eid);
        try {
            MimeMessage mimeMessage = emailSender.createMimeMessage();
            MimeMessageHelper mimeMessageHelper = new MimeMessageHelper(mimeMessage, true);

            mimeMessageHelper.setFrom("tjob.noreply@gmail.com");
            mimeMessageHelper.setTo("tjob.noreply@gmail.com");
            mimeMessageHelper.setSubject(subject);
...
//                    + "<p>Email: " + email + "</p>"
    ...
    public void sendEmailToJobSave(Job job, int eid) {
        Enterprise enterprise = enterpriseRepository.findByEid(eid);

        if (enterprise != null) {
            String enName = enterprise.getEnterprise_name(); // Tên công ty
            String jobTitle = job.getTitle();                // Tiêu đề công việc
            String toEmail = "tjob.noreply@gmail.com";         // Email của admin
            String name = enterprise.getEnterprise_name();                        // Tên người gửi (công ty)
            String subject = "New Job Post Pending Approval";

            // Nội dung email
            String body = String.format(
                    "Dear TJob Team,\n" +
                            "A new job post titled '%s' from the company '%s' has been submitted for approval.\n\n" +
                            "Please review and take the necessary actions.\n\n" +
                            "Best regards,\n" +
                            "%s",
                    jobTitle, enName, name
            );
            // Gửi email
            sendEmailToAdmin(eid, name, toEmail, subject, body);
        } else {
            System.err.println("Enterprise not found for eid: " + eid);
        }
    }
